...
alignas(32) constexpr uint64_t RC[ROUNDS * 4]{
  compute_rc(0),  compute_rc(0),  compute_rc(0),  compute_rc(0),
  compute_rc(1),  compute_rc(1),  compute_rc(1),  compute_rc(1),
  compute_rc(2),  compute_rc(2),  compute_rc(2),  compute_rc(2),
  compute_rc(3),  compute_rc(3),  compute_rc(3),  compute_rc(3),
  compute_rc(4),  compute_rc(4),  compute_rc(4),  compute_rc(4),
  compute_rc(5),  compute_rc(5),  compute_rc(5),  compute_rc(5),
  compute_rc(6),  compute_rc(6),  compute_rc(6),  compute_rc(6),
  compute_rc(7),  compute_rc(7),  compute_rc(7),  compute_rc(7),
  compute_rc(8),  compute_rc(8),  compute_rc(8),  compute_rc(8),
  compute_rc(9),  compute_rc(9),  compute_rc(9),  compute_rc(9),
  compute_rc(10), compute_rc(10), compute_rc(10), compute_rc(10),
  compute_rc(11), compute_rc(11), compute_rc(11), compute_rc(11),
  compute_rc(12), compute_rc(12), compute_rc(12), compute_rc(12),
  compute_rc(13), compute_rc(13), compute_rc(13), compute_rc(13),
  compute_rc(14), compute_rc(14), compute_rc(14), compute_rc(14),
  compute_rc(15), compute_rc(15), compute_rc(15), compute_rc(15),
  compute_rc(16), compute_rc(16), compute_rc(16), compute_rc(16),
  compute_rc(17), compute_rc(17), compute_rc(17), compute_rc(17),
  compute_rc(18), compute_rc(18), compute_rc(18), compute_rc(18),
  compute_rc(19), compute_rc(19), compute_rc(19), compute_rc(19),
  compute_rc(20), compute_rc(20), compute_rc(20), compute_rc(20),
  compute_rc(21), compute_rc(21), compute_rc(21), compute_rc(21),
  compute_rc(22), compute_rc(22), compute_rc(22), compute_rc(22),
  compute_rc(23), compute_rc(23), compute_rc(23), compute_rc(23),
};
...
#pragma message("Using AVX2 for keccak-[1600, 24] permutation")
  ...
  for (size_t i = 0; i < ROUNDS; i++) {
    ...
    const auto s19 =
      _mm256_permute4x64_epi64(s16, 0b10010011u); // s[19], s[16], s[17], s[18]
    const auto s14 =
      _mm256_permute4x64_epi64(s12, 0b11010010u); // s[14], s[12], s[13], s[15]
    const auto s9 =
      _mm256_permute4x64_epi64(s8, 0b11100001u); // s[9], s[8], s[10], s[11]
    ...
    const auto t13 =
      _mm256_permute4x64_epi64(c4_, 0b11100001u); // _, c'[4], _, _
    ...
    const auto t16 = _mm256_permute4x64_epi64(
      c0_4_, 0b00111001u);                     // c'[1], c'[2], c'[3], c'[0]
    const auto d0 = _mm256_xor_si256(c4, t16); // d[0], _, _, _

    const auto t17 = _mm256_permute4x64_epi64(d0, 0b00111001u); // _, _, _, d[0]
    const auto t18 =
      _mm256_blend_epi32(t17, d1_5, 0b00111111u); // d[1], d[2], d[3], d[0]
    const auto d0_4 =
      _mm256_permute4x64_epi64(t18, 0b10010011u); // d[0], d[1], d[2], d[3]

    const auto s0_ = _mm256_xor_si256(s0, d0_4); // s'[0], s'[1], s'[2], s'[3]
    const auto s5_ = _mm256_xor_si256(s5, d0_4); // s'[5], s'[6], s'[7], s'[8]
    const auto s10_ =
      _mm256_xor_si256(s10, d0_4); // s'[10], s'[11], s'[12], s'[13]
    const auto s15_ =
      _mm256_xor_si256(s15, d0_4); // s'[15], s'[16], s'[17], s'[18]
    const auto s20_ =
      _mm256_xor_si256(s20, d0_4); // s'[20], s'[21], s'[22], s'[23]

    const auto d4 =
      _mm256_permute4x64_epi64(d1_5, 0b10010011u); // d[4], d[1], d[2], d[3]
    ...
    const auto t39 =
      _mm256_permute4x64_epi64(s4__, 0b11001001u); // _, _, s'[4], _
    const auto t40 =
      _mm256_blend_epi32(s0__, t39, 0b00110000u); // s'[0], s'[1], s'[4], s'[3]
    const auto t41 = _mm256_permute4x64_epi64(
      t40, 0b10011100u); // s''[0], s''[5], s''[10], s''[15]

    const auto t42 =
      _mm256_permute4x64_epi64(s9__, 0b00111001u); // _, _, _, s'[9]
    const auto t43 =
      _mm256_blend_epi32(s5__, t42, 0b11000000u); // s'[5], s'[6], s'[7], s'[9]
    const auto t44 = _mm256_permute4x64_epi64(
      t43, 0b00101101u); // s''[1], s''[6], s''[11], s''[16]

    const auto t45 = _mm256_permute4x64_epi64(
      s10__, 0b01110010u); // s''[2], s''[7], s''[12], s''[17]

    const auto t46 = _mm256_blend_epi32(
      s15__, s19__, 0b00000011u); // s'[19], s'[16], s'[17], s'[18]
    const auto t47 = _mm256_permute4x64_epi64(
      t46, 0b10000111u); // s''[3], s''[8], s''[13], s''[18]

    const auto t48 =
      _mm256_permute4x64_epi64(s24__, 0b11100001u); // _, s'[24], _, _
    const auto t49 = _mm256_blend_epi32(
      s20__, t48, 0b00001100u); // s'[20], s'[24], s'[22], s'[23]
    const auto t50 = _mm256_permute4x64_epi64(
      t49, 0b11001001u); // s''[4], s''[9], s''[14], s''[19]

    const auto t51 =
      _mm256_permute4x64_epi64(s0__, 0b10101010u); // s''[20], _, _, _
    const auto t52 =
      _mm256_permute4x64_epi64(s5__, 0b11111111u); // s''[21], _, _, _
    const auto t53 = s14__;                        // s''[22], _, _, _
    const auto t54 = s15__;                        // s''[23], _, _, _
    const auto t55 =
      _mm256_permute4x64_epi64(s20__, 0b01010101u); // s''[24], _, _, _

    // χ step mapping

    const auto t56 = _mm256_andnot_si256(t44, t45);
    const auto t57 =
      _mm256_xor_si256(t41, t56); // s'''[0], s'''[5], s'''[10], s'''[15]

    const auto t58 = _mm256_andnot_si256(t45, t47);
    const auto t59 =
      _mm256_xor_si256(t44, t58); // s'''[1], s'''[6], s'''[11], s'''[16]

    const auto t60 = _mm256_andnot_si256(t47, t50);
    const auto t61 =
      _mm256_xor_si256(t45, t60); // s'''[2], s'''[7], s'''[12], s'''[17]

    const auto t62 = _mm256_andnot_si256(t50, t41);
    const auto t63 =
      _mm256_xor_si256(t47, t62); // s'''[3], s'''[8], s'''[13], s'''[18]

    const auto t64 = _mm256_andnot_si256(t41, t44);
    const auto t65 =
      _mm256_xor_si256(t50, t64); // s'''[4], s'''[9], s'''[14], s'''[19]

    const auto t66 =
      _mm256_xor_si256(t51, _mm256_andnot_si256(t52, t53)); // s'''[20], _, _, _
    const auto t67 =
      _mm256_xor_si256(t52, _mm256_andnot_si256(t53, t54)); // s'''[21], _, _, _
    const auto t68 =
      _mm256_xor_si256(t53, _mm256_andnot_si256(t54, t55)); // s'''[22], _, _, _
    const auto t69 =
      _mm256_xor_si256(t54, _mm256_andnot_si256(t55, t51)); // s'''[23], _, _, _
    const auto t70 =
      _mm256_xor_si256(t55, _mm256_andnot_si256(t51, t52)); // s'''[24], _, _, _

    const auto t71 = _mm256_permute4x64_epi64(
      t59, 0b10010011u); // s'''[16], s'''[1], s'''[6], s'''[11]
    const auto t72 = _mm256_permute4x64_epi64(
      t61, 0b01001110u); // s'''[12], s'''[17], s'''[2], s'''[7]
    const auto t73 = _mm256_permute4x64_epi64(
      t63, 0b00111001u); // s'''[8], s'''[13], s'''[18], s'''[3]

    const auto t74 = _mm256_blend_epi32(
      t57, t71, 0b11001100u); // s'''[0], s'''[1], s'''[10], s'''[11]
    const auto t75 = _mm256_blend_epi32(
      t72, t73, 0b11001100u); // s'''[12], s'''[13], s'''[2], s'''[3]
    s0 = _mm256_blend_epi32(
      t74, t75, 0b11110000u); // s'''[0], s'''[1], s'''[2], s'''[3]

    const auto t76 = _mm256_blend_epi32(
      t65, t57, 0b11001100u); // s'''[4], s'''[5], s'''[14], s'''[15]
    const auto t77 = _mm256_blend_epi32(
      t71, t72, 0b11001100u); // s'''[16], s'''[17], s'''[6], s'''[7]
    s4 = _mm256_blend_epi32(
      t76, t77, 0b11110000u); // s'''[4], s'''[5], s'''[6], s'''[7]

    const auto t78 = _mm256_blend_epi32(
      t65, t73, 0b00110011u); // s'''[8], s'''[9], s'''[18], s'''[19]
    s8 = _mm256_blend_epi32(
      t78, t74, 0b11110000u); // s'''[8], s'''[9], s'''[10], s'''[11]

    s12 = _mm256_blend_epi32(
      t75, t76, 0b11110000u); // s'''[12], s'''[13], s'''[14], s'''[15]
    s16 = _mm256_blend_epi32(
      t77, t78, 0b11110000u); // s'''[16], s'''[17], s'''[18], s'''[19]

    const auto t79 =
      _mm256_permute4x64_epi64(t67, 0b11100001u); // _, s'''[21], _, _
    const auto t80 =
      _mm256_permute4x64_epi64(t68, 0b11001001u); // _, _, s'''[22], _
    const auto t81 =
      _mm256_permute4x64_epi64(t69, 0b00111001u); // _, _, _, s'''[23]
    const auto t82 =
      _mm256_blend_epi32(t66, t79, 0b00001100u); // s'''[20], s'''[21], _, _
    const auto t83 = _mm256_blend_epi32(
      t82, t80, 0b00110000u); // s'''[20], s'''[21], s'''[22], _
    s20 = _mm256_blend_epi32(
      t83, t81, 0b11000000u); // s'''[20], s'''[21], s'''[22], s'''[23]
    s24 = t70;                // s'''[24], _, _, _

    // ι step mapping

    const auto t84 = _mm256_load_si256(
      (__m256i*)(RC + i * 4)); // RC[4*i], RC[4*i], RC[4*i], RC[4*i]
    const auto t85 = _mm256_xor_si256(s0, t84); // s''''[0], _, _, _
    s0 = _mm256_blend_epi32(
      t85, s0, 0b11111100u); // s''''[0], s'''[1], s'''[2], s'''[3]
  }
