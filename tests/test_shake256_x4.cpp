...
// Verify each lane of SHAKE256 x4 matches the scalar SHAKE256 output.
TEST(Sha3XOF, SHAKE256x4ParityWithScalar)
{
  for (size_t mlen = MIN_MSG_LEN; mlen < MAX_MSG_LEN; mlen++) {
    for (size_t olen = MIN_OUT_LEN; olen < MAX_OUT_LEN; olen++) {
      ...
      // scalar path
      auto shake256x1 = [&](std::span<const uint8_t> msg, std::span<uint8_t> out) {
        shake256::shake256_t h;
      ...
      shake256x1(msg0, out0_sc);
      shake256x1(msg1, out1_sc);
      shake256x1(msg2, out2_sc);
      shake256x1(msg3, out3_sc);
 ...
 * should yield same output bytes, for SHAKE256 x4 XOF.
 *
 * Uses msg0's content to determine chunk sizes (same across all 4 lanes).
 */
TEST(Sha3XOF, SHAKE256x4IncrementalAbsorptionAndSqueezing)
{
  for (size_t mlen = MIN_MSG_LEN; mlen < MAX_MSG_LEN; mlen++) {
    for (size_t olen = MIN_OUT_LEN; olen < MAX_OUT_LEN; olen++) {
      std::vector<uint8_t> msg0(mlen);
...
TEST(Sha3XOF, SHAKE256x4AllSameInput)
{
